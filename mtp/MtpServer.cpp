...
bool MtpServer::handleRequest() {
    ...
    if (mSendObjectHandle != kInvalidObjectHandle && operation != MTP_OPERATION_SEND_OBJECT) {
        // The host gave up after SendObjectInfo.  Report the transfer as
        // failed so the database releases its send guard and drops the entry
        // reserved for a file that never arrived.
        mDatabase->endSendObject(mSendObjectHandle, false);
        mSendObjectHandle = kInvalidObjectHandle;
        mSendObjectFormat = 0;
        mSendObjectModifiedTime = 0;
    }
...
MtpResponseCode MtpServer::doSendObjectInfo() {
    ...
    if (format == MTP_FORMAT_ASSOCIATION) {
        ...
        if (ret) {
            // Release the send guard that beginSendObject took above.
            mDatabase->endSendObject(handle, false);
            return MTP_RESPONSE_GENERAL_ERROR;
        }
